...
@Component({
  selector: 'app-addinventory-healthcare',
  templateUrl: './addinventory-healthcare.component.html',
  styleUrls: ['./addinventory-healthcare.component.scss']
})
export class AddinventoryHealthcareComponent implements OnInit {
  ...
  public categorySearchValue :any;
  public closeResult: string;

  public data: any[] = [
    {
      text: 'Foods', id: 1, items: [
          { text: 'Noodles,Pasta & Batter', id: 2 , items : [{ text: 'Canned Food', id: 1 }]},
          { text: 'Canned Food', id: 3 },
          { text: 'Occasional Furniture', id: 4 }
        ]
    },
    // {
    //   text: 'Foods', id: 2, items: [
    //     { text: 'Noodles,Pasta & Batter', id: 2 },
    //     { text: 'Canned Food', id: 3 },
    //     { text: 'Occasional Furniture', id: 4 }
    //   ]
    // }
  ...
  public listSubCategoryLit: Array<string> = ['FaceWash', 'ToothPaste', 'Panjaka Kasthuri','Face Mask'];
  public listPolicy: Array<string> = ['Policy 1', 'Policy 2', 'Policy 3','Policy 4'];
  public listType: any= [
    {id :'Normal' , value : 'Normal'},
    {id : 'Combo Offer' , value : 'Combo Offer'},
    {id :'Buy n Get n' , value : 'Buy n Get n'}
  ];
  ...
  public textvalue = ['614f011ce5e187385b61bcf4'];
  ...
  setEditData(data){
    ...
    this._addInvetoryService.get_all_product_listing('healthcare').subscribe((res:any) => {
    ...
    this.substitutionForm.patchValue({
      ...
      //substitution: data.substitutions,
      //pricing: ['',Validators.required],
    })
  ...
  getInventoryID(){
    ...
    this._addInvetoryService.get_product_id_count(type).subscribe((res:any) => {
        if(res.data){
          this.medimallID = 'HCP-02-00'+ res.data.count;
        }
    })
  }
  ...
  public itemDisabled(dataItem: any, index: number) {
    if(dataItem.subCategories !=[]){
      return dataItem.title; // disable the 2nd root item
    }
  }
  ...
  handleFilter(value) {
    let data = this.listSubCategoryLit.filter(
      (s) => s.toLowerCase().indexOf(value.toLowerCase()) !== -1
    );
    this.listSubCategoryLit = data;
  }
  ...
  save(pricingForm :any = null,tabRef){
    ...
    for(let i=0;i<this.formArray.length;i++){
      ...
      this.formArray[i].specialPrice === '' || this.formArray[i].stock === '' || this.formArray[i].image.length === 0)
      return false;
    }
    ...
    let input :any;
      ...
      formData.append('type','healthcare');
      ...
      formData.append('offerType',this.basicinfoForm.get('type').value);
    ...
    this._addInvetoryService.add_inventory_product(formData).subscribe((res:any)=>{
      console.log(res);
        if(res.status){
          ...
          this._router.navigate(['/inventory/inventory-list/healthcare'])
        }
        else{
          this.addLoading = false;
            Swal.fire({
  ...
  public removeEventHandler(e: RemoveEvent,i:any): void {
        ...
        if(e.path[0].naturalHeight === parseInt(height) && e.path[0].naturalWidth === parseInt(width) ){
         ...
         // this.imageLoading = true;
          ...
          this._addInvetoryService.upload_image(formData).subscribe((res:any) => {
            console.log(res);
            if(res.status){
              //this.imagePreviews.push(res.data.image_path);
              //this.formArray[i].imagePreview.push(res.data.image_path)
      ...
      if (result.value) {
        ...
        this._addInvetoryService.delete_image(input).subscribe((res:any) => {
          ...
          if(res.status){
            ...
            //this.formArray[index].imagePreview.splice(Imageindex,1);
  ...
  public hasChildren(data): boolean {
